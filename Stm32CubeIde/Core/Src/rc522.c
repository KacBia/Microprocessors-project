...
{
	/* CS LOW */
	HAL_GPIO_WritePin(MFRC522_SDA_PORT,MFRC522_SDA_PIN,GPIO_PIN_RESET);

	  //< even though we are calling transfer frame once, we are really sending
	  //< two 8-bit frames  together  sending two 8 bit frames back to back
	...
	/* CS HIGH */
	HAL_GPIO_WritePin(MFRC522_SDA_PORT,MFRC522_SDA_PIN,GPIO_PIN_SET);
}
...
/*
 * @brief: AntennaOn
 * @note: Open antennas, each time you start
 * - or shut down the natural barrier between the transmitter should be at least 1ms interval
 * @param: None
 * @retval: None
 */
void AntennaOn(void)
...
/*
  * @brief: AntennaOff
  * @note:Close antennas, each time you start or shut
  * down the natural barrier between the transmitter should be at least 1ms interval
  * @param: None
  * @retval: None
 */
...
/*
 * @brief: MFRC522_Anticoll
 * @note: Anti-collision detection, reading selected card serial number card
 * @param: parameters: serNum - returns 4 bytes card serial number, the first 5 bytes for the checksum byte
 * @retval: the successful return MI_OK
 */
 ...
 * @retval: the successful return of card capacity
 */
 ...
 * @param: - Password Authentication Mode
...
/*
 * @brief: MFRC522_Write
 * @note: Write block data
 * @param: blockAddr - block address; writeData - to 16-byte data block write
 * @retval: the successful return MI_OK
 */
